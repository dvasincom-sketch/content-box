/**
 * Разовый сид: публикация-«Страница» «Jimin» (шаблон 'profile') с ПОЛНЫМ
 * переписанным текстом из scripts/jimin-profile.json. Пишется в
 * publications.profile как blocks[] (Шапка, Быстрые факты, Хронология,
 * текстовые разделы, Аккордеон, Постеры, Плитки). Обновляет существующую
 * публикацию по slug, не плодит копии. Портрет (обложка), галерею и видео
 * добавляйте в студии.
 *
 * Запуск:
 *   node --env-file=.env --import=tsx scripts/seed-jimin-profile.ts [subdomain]
...
const profile = JSON.parse(readFileSync(join(__dir, 'jimin-profile.json'), 'utf8'))
const LEAD: string = profile.lead || 'Пак Чимин — ведущий вокалист и ведущий танцор BTS.'
...
async function main() {
  ...
  const data: any = {
    title: 'Jimin', slug: SLUG, tenant: tenant.id,
