...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   ALTER TYPE "public"."enum_site_settings_home_sections_type" ADD VALUE 'search' BEFORE 'latest';`)
}
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  CREATE TYPE "public"."enum_site_settings_home_sections_type" AS ENUM('hero', 'heroTeam', 'news', 'latest', 'popular', 'discussed', 'posterRows', 'categories', 'popularCategories', 'whyUs', 'socials', 'broadcast');
