...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   ALTER TABLE "publications" ADD COLUMN "external_ref" varchar;
  CREATE INDEX "publications_external_ref_idx" ON "publications" USING btree ("external_ref");
  ALTER TABLE "site_settings" ADD COLUMN "external_api_key_hash" varchar;
  CREATE INDEX "site_settings_external_api_key_hash_idx" ON "site_settings" USING btree ("external_api_key_hash");
  ALTER TABLE "site_settings" ADD COLUMN "external_api_key_prefix" varchar;
  ALTER TABLE "site_settings" ADD COLUMN "external_api_key_created_at" timestamp(3) with time zone;
  ALTER TABLE "site_settings" ADD COLUMN "external_api_key_last_used_at" timestamp(3) with time zone;`)
}
