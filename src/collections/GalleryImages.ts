...
export const GalleryImages: CollectionConfig = {
  ...
  fields: [
    ...
    {
      // Публикация, через композер которой изображение было загружено. Ставится
      // автоматически при загрузке из редактора публикации — даёт бесплатную
      // организацию «по публикациям» без ручных папок. ON DELETE set null:
      // удаление публикации не трогает картинку (она могла переиспользоваться).
      name: 'sourcePublication',
      type: 'relationship',
      relationTo: 'publications',
      label: 'Источник (публикация)',
      admin: {
        description: 'Публикация, через которую изображение загружено. Проставляется автоматически.',
      },
    },
  ],
  timestamps: true,
