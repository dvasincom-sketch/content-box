...
export default async function NewslettersAnalytics() {
  ...
  const barMax = Math.max(1, free, ...perTier.map((t) => t.count))
  ...
  return (
    <>
      ...
      <section className="studio-card">
        <div className="an__list-head"><Users size={16} /> Аудитория рассылки</div>
        ...
        <ul className="an__rows" style={{ marginTop: '1rem' }}>
          {perTier.map((t) => (
            <li key={t.id} className="an__row">
              <span className="an__row-bar" style={{ width: `${(t.count / barMax) * 100}%` }} aria-hidden />
              <span className="an__row-label">{t.name}</span>
              <span className="an__row-count">{t.count}</span>
            </li>
          ))}
          <li className="an__row">
            <span className="an__row-bar" style={{ width: `${(free / barMax) * 100}%`, opacity: 0.5 }} aria-hidden />
            <span className="an__row-label">Бесплатные</span>
            <span className="an__row-count">{free}</span>
          </li>
        </ul>
      </section>

      <section className="studio-card" style={{ marginTop: '1rem' }}>
        <div className="an__list-head"><Mail size={16} /> Реакция на дайджесты</div>
        {!listmonkEnabled() ? (
          <div className="an__empty">Подключение к системе рассылок настраивается платформой. Как только дайджесты начнут отправляться через неё, здесь появятся открытия и клики по каждому выпуску — с разбивкой по тарифам.</div>
        ) : digests.length === 0 ? (
          <div className="an__empty">Данных пока нет. Статистика появится после первого дайджеста, отправленного через систему рассылок.</div>
        ) : (
          <div className="nl-table">
            ...
            {digests.map((d) => (
              <div key={d.id} className="nl-row">
                <span className="nl-row__subj" title={d.subject}>{d.subject}{d.segment ? ` · ${d.segment}` : ''}</span>
                <span>{d.sent}</span>
