...
export const POST = withAuthor(async ({ req, payload, tenantId, author }) => {
  ...
  let uid: string
  try {
    // Яндекс.Диск отдаёт ссылку на страницу — конвертируем в прямую через их API.
    // Прочие URL (Object Storage/S3/R2) возвращаются как есть.
    ...
    return apiError(`${errorMessage(e, 'не удалось начать загрузку')}`, 502)
  }
