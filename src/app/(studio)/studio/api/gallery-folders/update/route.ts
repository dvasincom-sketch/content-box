...
export async function POST(req: NextRequest) {
  ...
  if ('parentId' in data) {
    if (data.parentId == null || data.parentId === '') {
      ...
      const okParent = await belongsToTenant(payload, 'gallery-folders', newParentId, tenantId)
      if (!okParent) {
...
async function isDescendantOf(
  ...
  while (currentId != null && hops < 100) {
    ...
    const doc: any = await payload
      .findByID({ collection: 'gallery-folders', id: currentId, depth: 0, overrideAccess: true })
      .catch(() => null)
    if (!doc) return false
    const t = doc.tenant && typeof doc.tenant === 'object' ? doc.tenant.id : doc.tenant
