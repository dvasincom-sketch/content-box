...
export const POST = withAuthor(async ({ req, payload, tenantId, author }) => {
  ...
  try {
    ...
    // Тип раздела («Обычный» / «Страница») — только явная настройка автора и
    // сохранением публикации НЕ меняется. Раньше здесь профиль принудительно
    // переводил раздел в pageMode, из-за чего обычный раздел-список после
    // редактирования публикации неожиданно превращался в «страницу».
    return apiOk()
  } catch (e: unknown) {
