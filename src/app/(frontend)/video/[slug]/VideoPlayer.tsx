...
export function VideoPlayer({
  ...
  const [selfSrc, setSelfSrc] = useState<{ master: string; poster: string | null; watermark: string | null; sprite: string | null; subtitles: { lang: string; label: string; url: string }[] | null; chapters: { start: number; title: string }[] | null } | null>(null)
      ...
      try {
        ...
        if (json.provider === 'audio') {
          ...
          if (json.status === 'ready' && typeof json.master === 'string') {
            setSelfSrc({ master: json.master, poster: typeof json.poster === 'string' ? json.poster : null, watermark: typeof json.watermark === 'string' ? json.watermark : null, sprite: typeof json.sprite === 'string' ? json.sprite : null, subtitles: Array.isArray(json.subtitles) ? json.subtitles : null, chapters: Array.isArray(json.chapters) ? json.chapters : null })
          } else {
            setProcessing(true)
          }
    ...
    >
      ...
      >
        {gate ? (
          ...
          <SelfHostedPlayer master={selfSrc.master} poster={selfSrc.poster} watermarkText={selfSrc.watermark} sprite={selfSrc.sprite} subtitles={selfSrc.subtitles} videoId={videoId} chapters={selfSrc.chapters} />
        ) : !src ? (
          <div
